...
import type { TaskTypeFormPayload } from '../../api/taskTypesAdminService';

interface TaskTypeFormProps {
  idPrefix: string;
  initialValues?: TaskTypeFormPayload;
  onSubmit: (values: TaskTypeFormPayload) => Promise<void> | void;
  onCancel: () => void;
...
const DEFAULT_VALUES: TaskTypeFormPayload = { label: '', colorHex: '#8b5cf6', order: 0 };
...
export default function TaskTypeForm({
  ...
  const [formData, setFormData] = useState<TaskTypeFormPayload>(initialValues);
    ...
    <form onSubmit={handleSubmit} className="space-y-4 pt-2">
      <FormField label="Name" htmlFor={`${idPrefix}-label`}>
        <Input
          ...
          required
          type="text"
          placeholder="E.g: Academic, Hobby, Work"
      ...
      <ColorPickerField
        id={`${idPrefix}-color`}
        value={formData.colorHex ?? '#8b5cf6'}
